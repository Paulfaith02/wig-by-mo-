...
import { faArrowRight, faAward, faChevronDown, faClock, faFilter, faHeart, faMapMarkerAlt, faPhone, faSearch, faShieldAlt, faStar, faTimes, faTruck, faWandMagicSparkles } from '@fortawesome/free-solid-svg-icons'
...
function Home({ wishlist, toggleWishlist, openQuick }) {
  ...
  return <motion.div {...pageMotion}>
    ...
    <section className="hero">
      <img className="hero-bg" src="/images/hero-luxury.png" alt="Wig By Mo bone straight luxury wig" />
      ...
      <div className="hero-content container">
        <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: .8, delay: .15 }}>
          ...
          <div className="hero-proof"><div><strong>100%</strong><span>Premium hair</span></div><div><strong>5★</strong><span>Client rated</span></div><div><strong>NG</strong><span>Nationwide delivery</span></div></div>
        </motion.div>
      </div>
      <div className="scroll-cue"><span>Discover</span><i /></div>
    </section>

    <section className="trust-strip"><div className="container trust-grid"><div><FontAwesomeIcon icon={faTruck}/><span><strong>Nationwide delivery</strong><small>Tracked to your door</small></span></div><div><FontAwesomeIcon icon={faShieldAlt}/><span><strong>Quality assured</strong><small>Every unit inspected</small></span></div><div><FontAwesomeIcon icon={faWandMagicSparkles}/><span><strong>Ready to wear</strong><small>Customized by experts</small></span></div><div><FontAwesomeIcon icon={faWhatsapp}/><span><strong>Personal support</strong><small>Real styling advice</small></span></div></div></section>
    ...
    <section className="editorial-section">
      <div className="editorial-image"><img src="/images/blunt-bob.png" alt="Wig By Mo blunt cut bob" loading="lazy"/><span className="vertical-note">Precision · Polish · Presence</span></div>
    ...
    <section className="section testimonials"><SectionHead kicker="Client love" title="The confidence is mutual." text="Real words from women in the Wig By Mo community."/>
      <Swiper modules={[Autoplay, Pagination]} autoplay={{ delay: 4500 }} pagination={{ clickable: true }} spaceBetween={24} breakpoints={{ 800: { slidesPerView: 2 }, 1100: { slidesPerView: 3 } }}>
        {reviews.map((review) => <SwiperSlide key={review.id}><article className="review-card"><div className="stars">{[1,2,3,4,5].map(i => <FontAwesomeIcon key={i} icon={faStar}/>)}</div><blockquote>“{review.quote}”</blockquote><div><strong>{review.name}</strong><span>{review.city} · {review.product}</span></div></article></SwiperSlide>)}
      </Swiper>
    </section>
...
function Shop({ wishlist, toggleWishlist, openQuick }) {
  ...
  const filtered = useMemo(() => products.filter(p => (category === 'all' || p.category === category) && p.name.toLowerCase().includes(query.toLowerCase())).sort((a,b) => sort === 'low' ? a.price-b.price : sort === 'high' ? b.price-a.price : Number(b.featured)-Number(a.featured)), [query, category, sort])
  return <motion.div {...pageMotion}>
...
function ProductDetails({ wishlist, toggleWishlist }) {
  ...
  const orderUrl=`https://wa.me/${whatsapp}?text=${encodeURIComponent(`Hello Wig By Mo, I would like to order ${product.name}${length?` (${length})`:''}${color?` in ${color}`:''}.`)}`
  return <motion.div {...pageMotion} className="product-page">
    ...
    <section className="product-detail container"><div className="detail-gallery"><div className="main-detail-image"><img src={product.image} alt={product.name}/>{product.badge&&<span className="badge">{product.badge}</span>}</div><div className="thumbs"><button className="active"><img src={product.image} alt="Front view"/></button><button><img src={product.image} alt="Detail view"/></button></div></div>
      <div className="detail-copy"><span className="eyebrow">Premium collection</span><h1>{product.name}</h1><div className="rating"><span><FontAwesomeIcon icon={faStar}/><FontAwesomeIcon icon={faStar}/><FontAwesomeIcon icon={faStar}/><FontAwesomeIcon icon={faStar}/><FontAwesomeIcon icon={faStar}/></span> 5.0 · Client favourite</div><p className="detail-price">{formatPrice(product.price)}</p><p className="detail-desc">{product.description}</p>
        ...
        <div className="detail-actions"><a className="btn btn-gold" href={orderUrl} target="_blank" rel="noreferrer"><FontAwesomeIcon icon={faWhatsapp}/> Order on WhatsApp</a><button className={`detail-wish ${wishlist.includes(product.id)?'active':''}`} onClick={()=>toggleWishlist(product.id)}><FontAwesomeIcon icon={faHeart}/></button></div>
        <div className="assurance"><div><FontAwesomeIcon icon={faTruck}/><span><strong>Fast, tracked delivery</strong><small>Nationwide across Nigeria</small></span></div><div><FontAwesomeIcon icon={faShieldAlt}/><span><strong>Quality checked</strong><small>Inspected before dispatch</small></span></div></div>
...
function Gallery() { return <motion.div {...pageMotion}><Seo title="Gallery" description="Explore Wig By Mo transformations, installations and signature looks."/><PageHero kicker="The lookbook" title="Beautiful hair, beautifully worn." text="A glimpse into our textures, transformations and the women who make each look their own."/><section className="section container"><div className="masonry">{[...gallery,...gallery].map((g,i)=><motion.figure key={i} className={g.className||''} initial={{opacity:0,y:20}} whileInView={{opacity:1,y:0}} viewport={{once:true}}><img src={g.image} alt={g.label}/><figcaption><span>{String(i+1).padStart(2,'0')}</span><strong>{g.label}</strong></figcaption></motion.figure>)}</div></section></motion.div> }

function About() { return <motion.div {...pageMotion}><Seo title="Our Story" description="Meet Wig By Mo, a Nigerian luxury wig brand built around quality, ease and confidence."/><PageHero kicker="Our story" title="Luxury should feel like you." text="Not a costume. Not a compromise. Just exceptional hair that lets your confidence take the lead."/><section className="story container"><div className="story-image"><img src="/images/deep-wave.png" alt="Wig By Mo luxury curly wig"/></div><div className="story-copy"><span className="eyebrow">The woman behind the name</span><h2>Created with care.<br/><em>Curated with intention.</em></h2><p>Wig By Mo began with one simple belief: every woman deserves hair that feels as good as it looks. No uncertainty, no disappointing textures, no complicated installs.</p><p>We source thoughtfully, inspect every unit and finish each detail with the woman wearing it in mind. The result is a collection made for real life—beautifully.</p><div className="signature">Mo <span>Founder, Wig By Mo</span></div></div></section><section className="values"><div className="container"><SectionHead kicker="What guides us" title="The details are the difference." light/><div className="value-grid"><article><span>01</span><h3>Quality, always</h3><p>We select hair for softness, longevity, movement and a finish that remains beautiful.</p></article><article><span>02</span><h3>Made personal</h3><p>We listen first, then help you choose the fit, texture and finish that suits you.</p></article><article><span>03</span><h3>Effortless beauty</h3><p>Our units are designed to simplify your routine without compromising your standard.</p></article></div></div></section></motion.div> }

function FAQ() { const [open,setOpen]=useState(0); return <motion.div {...pageMotion}><Seo title="Frequently Asked Questions" description="Answers about Wig By Mo orders, delivery, hair care, returns and installation."/><PageHero kicker="Client care" title="Everything you need to know." text="Clear answers for choosing, ordering and caring for your Wig By Mo unit."/><section className="faq-wrap container"><div className="faq-aside"><span className="eyebrow">Still curious?</span><h2>We’re here to help.</h2><p>Chat directly with our team for personal hair advice or order support.</p><a className="btn btn-dark" href={`https://wa.me/${whatsapp}`} target="_blank" rel="noreferrer"><FontAwesomeIcon icon={faWhatsapp}/> Ask us anything</a></div><div className="faq-list">{faqs.map((f,i)=><article key={f.q} className={open===i?'open':''}><button onClick={()=>setOpen(open===i?-1:i)}><span><small>{f.group}</small>{f.q}</span><FontAwesomeIcon icon={faChevronDown}/></button><AnimatePresence>{open===i&&<motion.p initial={{height:0,opacity:0}} animate={{height:'auto',opacity:1}} exit={{height:0,opacity:0}}>{f.a}</motion.p>}</AnimatePresence></article>)}</div></section></motion.div> }

function Contact() { const { register, handleSubmit, reset, formState:{errors} }=useForm({defaultValues:JSON.parse(localStorage.getItem('wigbymo-contact-draft')||'{}')}); const submit=data=>{localStorage.removeItem('wigbymo-contact-draft');toast.success('Your message is ready — we’ll be in touch.');reset();}; return <motion.div {...pageMotion}><Seo title="Contact" description="Contact Wig By Mo for wig consultations, orders, installation and support in Lagos."/><PageHero kicker="Private client care" title="Let’s talk about your hair." text="For advice, custom requests or appointment bookings, our team is ready to help."/><section className="contact-wrap container"><div className="contact-details"><span className="eyebrow">Visit or message us</span><h2>We’d love to hear from you.</h2><p>Appointments give us time to understand the look you want and guide you without rushing.</p><div className="contact-method"><FontAwesomeIcon icon={faWhatsapp}/><span><small>WhatsApp</small><strong>+234 800 000 0000</strong></span></div><div className="contact-method"><FontAwesomeIcon icon={faPhone}/><span><small>Phone</small><strong>+234 800 000 0000</strong></span></div><div className="contact-method"><FontAwesomeIcon icon={faMapMarkerAlt}/><span><small>Studio</small><strong>Lagos, Nigeria · By appointment</strong></span></div><div className="contact-method"><FontAwesomeIcon icon={faClock}/><span><small>Hours</small><strong>Mon–Sat · 9:00am–6:00pm</strong></span></div></div><form className="contact-form" onSubmit={handleSubmit(submit)} onChange={e=>localStorage.setItem('wigbymo-contact-draft',JSON.stringify(Object.fromEntries(new FormData(e.currentTarget))))}><div className="form-head"><span>01</span><h3>Tell us what you’re looking for</h3></div><label>Full name<input {...register('name',{required:true})} placeholder="Your name"/>{errors.name&&<small>Please enter your name</small>}</label><label>Phone / WhatsApp<input {...register('phone',{required:true})} placeholder="+234"/>{errors.phone&&<small>Please enter your phone number</small>}</label><label>Email address<input type="email" {...register('email')} placeholder="you@example.com"/></label><label>How can we help?<select {...register('interest')}><option>Help choosing a wig</option><option>Place an order</option><option>Book an installation</option><option>Custom request</option></select></label><label>Tell us more<textarea {...register('message')} rows="5" placeholder="The style, length or occasion you have in mind..."/></label><button className="btn btn-gold">Send enquiry <FontAwesomeIcon icon={faArrowRight}/></button></form></section></motion.div> }
